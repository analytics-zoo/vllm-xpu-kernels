"""Compare mixed per-sequence causal attention with a split baseline."""
import json
import statistics
import time
...
def main():
    ...
    def split():
        outputs, offset = [], 0
        for row, (ql, kl, is_causal) in enumerate(zip(q_lens, kv_lens,
                                                      causal)):
            outputs.append(
                flash_attn_varlen_func(
                    ...
                    v,
                    ql,
                    torch.tensor([0, ql], dtype=torch.int32),
                    kl,
    ...
    torch.testing.assert_close(candidate_out,
                               split_out,
                               atol=1.5e-2,
                               rtol=1.5e-2)
    for _ in range(10):
    ...
    print(
        json.dumps(
            {
                "baseline_split_ms": split_ms,
                "candidate_mixed_ms": mixed_ms,
                "baseline_median_ms": statistics.median(split_ms),
                "candidate_median_ms": statistics.median(mixed_ms),
                "ratio":
                statistics.median(mixed_ms) / statistics.median(split_ms)
            },
            sort_keys=True))
