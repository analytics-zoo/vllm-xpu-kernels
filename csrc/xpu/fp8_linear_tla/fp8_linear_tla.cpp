...
template <int Splits> class NtSplitKGemm;
template <int Splits> class NtSplitKReduce;
...
at::Tensor fp8_gemm_impl(
  ...
  TORCH_CHECK(weight.device() == input.device() && weight.dim() == 2 &&
                  weight.scalar_type() == at::ScalarType::Float8_e4m3fn &&
                  weight.size(0) == k && weight.stride(0) == 1 &&
                  weight.stride(1) == k,
              "linear_tla_unsupported: FP8 B[K,N] stride(1,K) required");
  const int64_t n = weight.size(1);
  TORCH_CHECK(m >= 1 && m <= 8 && k >= kSplits * kTileK &&
                  k <= 65536 && k % kTileK == 0 &&
                  n >= kTileN && n <= 65536 && n % kTileN == 0,
              "linear_tla_unsupported: M1..8, K%32=0/K>=256, N%64=0 required");
  TORCH_CHECK(!bias.has_value() && scale.has_value() &&
  ...
  queue.parallel_for<NtSplitKGemm<kSplits>>(
      ...
      props, [=](sycl::nd_item<3> item) {
        ...
        auto B = make_tensor(make_gmem_ptr(b),
                             make_layout(make_shape(int(n), int(k)),
                                         make_stride(int(k), _1{})));
...
at::Tensor fp8_gemm_w8a16(
  ...
  const bool large = (n == 4096 && k == 2816) ||
                     (n == 5120 && k == 2816);
  if (large) {
...
bool supported(const at::Tensor& a, const at::Tensor& b,
               const std::optional<at::Tensor>& scale,
               const std::optional<at::Tensor>& bias) {
  if (!a.is_xpu() || a.scalar_type() != at::kHalf || a.dim() != 2 ||
      ...
      b.size(0) != a.size(1) || b.stride(0) != 1 ||
      b.stride(1) != a.size(1) || bias.has_value() || !scale.has_value() ||
      scale->device() != a.device() || scale->scalar_type() != at::kFloat ||
      scale->numel() != 1 || !scale->is_contiguous()) {
    return false;
  }
...
at::Tensor fp8_gemm_dispatch(
  ...
  using Signature = at::Tensor(const at::Tensor&, const at::Tensor&,
...
at::Tensor fp8_gemm_meta(
  ...
  TORCH_CHECK(weight.dim() != 3 || input.dim() == 3,
...
TORCH_LIBRARY_FRAGMENT(_xpu_C, m) {
  m.def("fp8_gemm_w8a16_tla(Tensor A, Tensor B, Tensor? B_scale_, Tensor? bias_=None) -> Tensor");
  m.impl("fp8_gemm_w8a16_tla", torch::kXPU,
         &vllm::linear_tla::fp8_gemm_dispatch);
  m.impl("fp8_gemm_w8a16_tla", torch::kMeta,
         &vllm::linear_tla::fp8_gemm_meta);
}
